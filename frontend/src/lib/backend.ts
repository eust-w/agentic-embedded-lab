import type { AELRunRecord, AELRunRequest, AgentHandle, AgentResult, AgentSpec, AetherMemory, AttachmentRef, AutomationJob, AutomationSpec, BrowserConsoleEntry, BrowserNetworkEntry, BrowserStatus, GitChange, GitFileContent, GitPullRequest, HandoffResult, InstalledPlugin, Item, PermissionProfile, ProjectInfo, ReleaseResult, TerminalInfo, TerminalSnapshot, Thread, Turn } from '../types'
...
export interface BackendAPI {
  ...
  ChangeProjectPermission(permission: PermissionProfile): Promise<ProjectInfo>
  ...
  CreateThreadWithModel(projectID: string, title: string, permission: PermissionProfile, model: string): Promise<Thread>
  ...
  MessageAgent(id: string, message: string): Promise<Turn>
  SteerAgent(id: string, message: string): Promise<Turn>
  WaitAgent(id: string): Promise<AgentHandle>
  AgentResult(id: string): Promise<AgentResult>
  InterruptAgent(id: string): Promise<boolean>
  CloseAgent(id: string): Promise<boolean>
  HandoffAgent(id: string, cleanup: boolean): Promise<HandoffResult>
  ...
  BrowserDownload(url: string): Promise<string>
  ...
  ComputerTree(bundleID: string, limit: number): Promise<string>
  ComputerScreenshot(bundleID: string): Promise<string>
  ComputerClick(bundleID: string, x: number, y: number, confirmed: boolean): Promise<void>
  ComputerType(bundleID: string, text: string, confirmed: boolean): Promise<void>
  ...
  TriggerAutomations(eventSource: string): Promise<string[]>
  CancelAutomation(jobID: string): Promise<boolean>
  AutomationJob(jobID: string): Promise<AutomationJob>
